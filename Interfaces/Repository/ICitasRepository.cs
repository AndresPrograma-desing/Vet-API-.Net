...
public interface ICitasRepository
{
    ...
    Task<bool> AnyConflictAsync(int doctorId, DateTime fecha, TimeOnly hora);
    Task<MetodoPago?> GetMetodoPagoByNameAsync(string nombre);
    void AddMetodoPago(MetodoPago metodo);
    Task<Cita?> GetByIdWithMetodoPagoAsync(int id);
}